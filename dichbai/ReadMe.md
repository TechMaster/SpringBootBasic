# Danh sách những bài sinh viên lớp Spring 1A+1B cần dịch để chuẩn bị kiến thức đi phỏng vấn

Mỗi bạn trong lớp sẽ dịch 5 bài.

### Quy định khi dịch bài:
1. Hãy xem danh sách các bài viết rồi đăng ký với cô Thanh Thương
2. Ghi rõ link đến văn bản gốc, tên tiếng Anh đầy đủ
3. Ghi rõ họ và tên người dịch. Nên có cả email để nhà tuyển dụng email liên hệ bạn. Ảnh avatar lấy ảnh mặt thật, càng đẹp trai, xinh gái càng tốt. Cơ hội việc làm 1000$ đó !
4. Hightlight mã nguồn đầy đủ
5. Hãy viết với tinh thần chia sẻ, văn phong dí dỏm vui tươi. Nếu bạn thấy code tác giả khó hiểu hãy chú thích thêm. Bạn sẽ là Nam Loda.me tiếp theo !
6. Hãy để avatar ảnh mặt thật trong profile. Ảnh càng đẹp trai càng

Cuối khoá học, chúng ta sẽ làm một bài tổng kết rồi link để các bài dịch. Mục tiêu càng có nhiều người đọc bài viết của các bạn càng tốt --> Search Engine Optimization.

### Nên làm
1. Trong lúc dịch hãy cố gắng chạy thử code của tác giả. Dịch và học lập trình song song với nhau. Đây không nền là 2 việc tách rời.
Code ví dụ thường đã cũ, bạn có thể nâng cấp, hoặc cải tiến. Welcome !
2. Gửi bài cho bạn cùng học để review, sửa lỗi chính tả, cũng như đọc xem có dễ hiểu không
3. Giữ nguyên các từ tiếng Anh không có từ tiếng Việt tương đương, hoặc là từ khoá, tên class, tên method. Mục tiêu là người đọc dễ hiểu, chứ không nên dịch 1-1 máy móc
4. Hãy đọc bài viết của bạn và góp ý cho bạn. Hãy học hỏi từ bài viết của bạn. **Học thầy không tày học bạn !**


**Chúc các bạn dịch vui và tìm được việc làm Java Spring Boot ưng ý**

## Java
- [Java 8 Interview Questions](https://www.javainuse.com/java/java8_intvw)
- [Maven Interview Questions](https://www.javainuse.com/misc/maven-interview-questions)
- [Annotations in Java](https://ducmanhphan.github.io/2019-01-09-Annotations-in-java/)
- [Java Versions and Features](https://www.marcobehler.com/guides/a-guide-to-java-versions-and-features)
- [How To Work With Files In Java](https://www.marcobehler.com/guides/java-files)
- [How To Do Logging In Java](https://www.marcobehler.com/guides/java-logging)

### Java Features
- [Java 15 - Sealed Classes](https://www.logicbig.com/tutorials/core-java-tutorial/java-15-changes/sealed-classes-intro.html)
- [Java 14 - Records](https://www.logicbig.com/tutorials/core-java-tutorial/java-14-changes/records.html)
- [Java 14 - Pattern Matching for instanceof](https://www.logicbig.com/tutorials/core-java-tutorial/java-14-changes/instance-of-pattern-matching.html)
- [Java 14 - What we can and cannot do with Java 14 records?](https://www.logicbig.com/tutorials/core-java-tutorial/java-14-changes/restrictions-on-records.html)
- [Java 14 - Switch Expressions And Statements Examples](https://www.logicbig.com/tutorials/core-java-tutorial/java-14-changes/switch-expressions-and-statements.html)
- [Java 13 - Text Blocks](https://www.logicbig.com/tutorials/core-java-tutorial/java-13-changes/text-block-examples.html)
- [Java 12 - Switch Expression](https://www.logicbig.com/tutorials/core-java-tutorial/java-12-changes/switch-expression-examples.html)
- [Java 12 - String Changes](https://www.logicbig.com/tutorials/core-java-tutorial/java-12-changes/string-changes.html)
- [Java 12 - Compact Number Formatting support](https://www.logicbig.com/tutorials/core-java-tutorial/java-12-changes/compact-number-format.html)
- [Java 12 - Comparing files with Files.mismatch()](https://www.logicbig.com/tutorials/core-java-tutorial/java-12-changes/files-mismatch-method.html)
- [Java 11 - Local-Variable Syntax for Lambda Parameters](https://www.logicbig.com/tutorials/core-java-tutorial/java-11-changes/lambda-var-syntax.html)
- [Java 11 - Nested-Based Access Control](https://www.logicbig.com/tutorials/core-java-tutorial/java-11-changes/nested-based-access-control.html)
- [Java 11 - Getting started with HTTP Client API](https://www.logicbig.com/tutorials/core-java-tutorial/java-11-changes/http-client.html)
- [Java 11 - String Changes](https://www.logicbig.com/tutorials/core-java-tutorial/java-11-changes/string-changes.html)
## Spring Boot
- [What is Spring Framework? An Unorthodox Guide](https://www.marcobehler.com/guides/spring-framework)
- [Spring Boot – CRUD Application](https://howtodoinjava.com/spring-boot2/crud-application-thymeleaf/)
- [Configure a Spring Boot Web Application](https://www.baeldung.com/spring-boot-application-configuration)

### Dependency Injection
- [Spring - Quick Concepts and Example](https://www.logicbig.com/tutorials/spring-framework/spring-core/quick-start.html)
- [Spring - Defining Injection point by using @Inject annotation](https://www.logicbig.com/tutorials/spring-framework/spring-core/defining-injection-point-by-using-inject-annotation.html)
- [Spring - Java-based configuration: Using @Configuration](https://www.logicbig.com/tutorials/spring-framework/spring-core/java-config.html)
- [CommandLineRunner interface example](https://howtodoinjava.com/spring-boot/command-line-runner-interface-example/)
### Miscelaneous
- [Logging](https://www.tutorialspoint.com/spring_boot/spring_boot_logging.htm)
- [Interceptor](https://www.tutorialspoint.com/spring_boot/spring_boot_interceptor.htm)
- [Task Scheduling](https://www.tutorialspoint.com/spring_boot/spring_boot_scheduling.htm)
- [Send Email](https://www.tutorialspoint.com/spring_boot/spring_boot_sending_email.htm)
- [Spring boot – Send email with attachment](https://howtodoinjava.com/spring-boot2/send-email-with-attachment/)
- [Hytrix](https://www.tutorialspoint.com/spring_boot/spring_boot_hystrix.htm)
- [Spring Batch Tutorial: Batch Processing Made Easy with Spring](https://www.toptal.com/spring/spring-batch-tutorial) đọc bài này để hiểu cho dễ hơn [Sample dùng Spring Batch để xử lý dữ liệu đơn hàng dạng csv](https://viblo.asia/p/sample-dung-spring-batch-de-xu-ly-du-lieu-don-hang-dang-csv-E375zQw6lGW)
- [Loạt bài Spring Batch](https://howtodoinjava.com/spring-batch/) thấy bài nào hay thì dịch. Nhớ chạy code thử nhé !
- [Spring boot caching tutorial with example](https://howtodoinjava.com/spring-boot2/spring-boot-cache-example/)
- [Spring retry module example with spring boot](https://howtodoinjava.com/spring-boot2/spring-retry-module/)


### Session - Cookie
- [Spring cookies tutorial](http://zetcode.com/spring/cookies/ )
- [Guide to Spring Session](https://www.baeldung.com/spring-session)
- [Spring Boot + Session Management Hello World Example](https://www.javainuse.com/spring/springboot_session)
- [Control the Session with Spring Security](https://www.baeldung.com/spring-security-session)
### Groovy
Code cũng rất hay các bạn nhé. Mà lại giống Java
- [Spring Boot + Groovy: From Zero to Hero](https://dev.to/jorge_rockr/spring-boot--groovy-from-zero-to-hero-5d39)
- [Spring Boot + Groovy Templates CRUD Example](https://www.dariawan.com/tutorials/spring/spring-boot-groovy-templates-crud-example/)
- [Groovy - Class File Vs Script File](https://www.logicbig.com/tutorials/misc/groovy/class-vs-script-file.html)

### Phỏng vấn
- [Spring Boot Interview Questions](https://www.javainuse.com/spring/SpringBootInterviewQuestions)
- [11 Spring Boot Interview Questions That Make You Think](https://www.marcobehler.com/guides/spring-boot-interview-questions)

### Spring Boot REST
- [Setting Up Swagger 2 with a Spring REST API](https://www.baeldung.com/swagger-2-documentation-for-spring-rest-api)
- [Documenting a Spring REST API Using OpenAPI 3.0](https://www.baeldung.com/spring-rest-openapi-documentation)
- [Spring Boot - Enabling Swagger2](https://www.tutorialspoint.com/spring_boot/spring_boot_enabling_swagger2.htm)

### Configuration
- [How Spring Boot’s Autoconfigurations Work](https://www.marcobehler.com/guides/spring-boot)

### Transaction
- [Spring Transaction Management: @Transactional In-Depth](https://www.marcobehler.com/guides/spring-transaction-management-transactional-in-depth)
- [Managing Transactions with Spring and Spring Data JPA](https://thorben-janssen.com/transactions-spring-data-jpa/)
- [Using Transactions in Spring Data JPA](https://www.logicbig.com/tutorials/spring-framework/spring-data/transactions.html)

## Cơ sở dữ liệu quan hệ
- [What is Normalization? 1NF, 2NF, 3NF, BCNF Database Example](https://www.guru99.com/database-normalization.html)
- [Database Normalization](https://www.w3schools.in/dbms/database-normalization/)
- [How indexing works](https://dataschool.com/sql-optimization/how-indexing-works/)
- [Indexing in DBMS: What is, Types of Indexes with EXAMPLES](https://www.guru99.com/indexing-in-database.html)
- [Database Migrations with Flyway](https://www.baeldung.com/database-migrations-with-flyway)
- [Getting Started with Flyway and Version-Based Database Migration](https://thorben-janssen.com/flyway-getting-started/)
- [JDBC - a short guide](https://www.marcobehler.com/guides/jdbc)
- [11 important database designing rules which I follow](https://www.codeproject.com/Articles/359654/11-important-database-designing-rules-which-I-fo-2)

## JPA
- [Java & Databases: An Overview of Libraries & APIs](https://www.marcobehler.com/guides/java-databases)
- [JPA and Hibernate Tutorial using Spring Boot Data JPA](https://www.springboottutorial.com/introduction-to-jpa-with-spring-boot-data-jpa) Bài này cực dài, tính thành 2 bài.
- [How to persist creation and update timestamps with Hibernate](https://thorben-janssen.com/persist-creation-update-timestamps-hibernate/)
- [https://thorben-janssen.com/fetch-multiple-entities-id-hibernate/](https://thorben-janssen.com/fetch-multiple-entities-id-hibernate/)
- [Database Handling](https://www.tutorialspoint.com/spring_boot/spring_boot_database_handling.htm)
- [Native Queries – How to call native SQL queries with JPA & Hibernate](https://thorben-janssen.com/jpa-native-queries/)
- [Spring Data JPA Tutorial](https://www.stackchief.com/tutorials/Spring%20Data%20JPA%20Tutorial)
- [DataSource configuration](https://howtodoinjava.com/spring-boot2/datasource-configuration/)
- [Inheritance Strategies with JPA and Hibernate – The Complete Guide](https://thorben-janssen.com/complete-guide-inheritance-strategies-jpa-hibernate)
- [Hibernate Inheritance Mapping](https://www.baeldung.com/hibernate-inheritance)

### Các kiểu dữ liệu đặc biệt
- [Persisting Enums in JPA](https://www.baeldung.com/jpa-persisting-enums-in-jpa)
- [How to map a PostgreSQL ARRAY to a Java List with JPA and Hibernate](https://vladmihalcea.com/postgresql-array-java-list/)
- [Java Map to JSON mapping with JPA and Hibernate](https://vladmihalcea.com/java-map-json-jpa-hibernate/)
- [How to map a String JPA property to a JSON column using Hibernate](https://vladmihalcea.com/map-string-jpa-property-json-column-hibernate/)
- [Multidimensional array mapping with JPA and Hibernate](https://vladmihalcea.com/multidimensional-array-jpa-hibernate/)
- [How to map a PostgreSQL HStore entity property with JPA and Hibernate](https://vladmihalcea.com/map-postgresql-hstore-jpa-entity-property-hibernate/)
- [jOOQ - a short guide](https://www.marcobehler.com/guides/jooq)


## Unit Test
- [Spring Boot - Application Unit Testing with @SpringBootTest](https://www.logicbig.com/tutorials/spring-framework/spring-boot/boot-application-testing.html)
- [Difference Between @Mock and @MockBean](https://rieckpil.de/difference-between-mock-and-mockbean-spring-boot-applications/)
- [Fix No Qualifying Spring Bean Error For Spring Boot Tests](https://rieckpil.de/fix-no-qualifying-spring-bean-error-for-spring-boot-tests/)

### Mockito
- [Danh sách các bài hướng dẫn về Mockito](https://www.logicbig.com/tutorials/unit-testing/mockito.html)
- [Mockito - Creating Spy Of Real Objects](https://www.logicbig.com/tutorials/unit-testing/mockito/creating-spy-of-real-object.html)
- [Mockito - Verifying Method Calls](https://www.logicbig.com/tutorials/unit-testing/mockito/verify-method-calls.html)
- [Mockito - verifyNoMoreInteractions() and verifyNoInteractions()](https://www.logicbig.com/tutorials/unit-testing/mockito/verify-no-more-interactions.html)
- [Mockito - Stubbing a method's return Value](https://www.logicbig.com/tutorials/unit-testing/mockito/subbing-method-return-values.html)
- [Mockito - BDD Style Verification using then() - should()](https://www.logicbig.com/tutorials/unit-testing/mockito/bdd-style-behavior-verification.html)
- [Mockito - BDD Style Stubbing with given() - willReturn()](https://www.logicbig.com/tutorials/unit-testing/mockito/bdd-style-stubbing-with-given-will-return.html)
- [Mockito - Verifying Multiple Number of Method Invocations](https://www.logicbig.com/tutorials/unit-testing/mockito/verifying-varing-number-of-invocations.html)
- [Mockito - Stubbing methods with exceptions](https://www.logicbig.com/tutorials/unit-testing/mockito/stubbing-exceptions.html)
- [Mockito.mock() vs @Mock vs @MockBean](https://www.baeldung.com/java-spring-mockito-mock-mockbean)
- [Spring Boot - Testing With @MockBean](https://www.logicbig.com/tutorials/spring-framework/spring-boot/testing-with-mocking-beans.html)


## Security
- [Spring Security: Authentication and Authorization In-Depth](https://www.marcobehler.com/guides/spring-security)

### Loạt bài Spring Security trên https://www.baeldung.com/
- [Spring Security Form Login](https://www.baeldung.com/spring-security-login)
- [Spring Security Basic Authentication](https://www.baeldung.com/spring-security-basic-authentication)
- [Spring Security Logout](https://www.baeldung.com/spring-security-logout)
- [Login For a Spring Web App – Error Handling and Localization](https://www.baeldung.com/spring-security-login-error-handling-localization)
- [Redirect to Different Pages after Login with Spring Security](https://www.baeldung.com/spring_redirect_after_login)
- [Spring Security Remember Me](https://www.baeldung.com/spring-security-remember-me)
- [Spring Security Authentication Provider](https://www.baeldung.com/spring-security-authentication-provider)
- [How to Manually Authenticate User with Spring Security](https://www.baeldung.com/manually-set-user-authentication-spring-security)
- [Retrieve User Information in Spring Security](https://www.baeldung.com/get-user-in-spring-security)
- [Spring Security – Roles and Privileges](https://www.baeldung.com/role-and-privilege-for-spring-security-registration)
- [Spring Boot – OAuth2 Authorization and Resource Servers](https://howtodoinjava.com/spring-boot2/oauth2-auth-server/)
- [CORS Support](https://www.tutorialspoint.com/spring_boot/spring_boot_cors_support.htm)
- [What is JWT?](https://www.javainuse.com/spring/jwt)
- [How to Set Up Java Spring Boot JWT Authorization and Authentication](https://www.freecodecamp.org/news/how-to-setup-jwt-authorization-and-authentication-in-spring/)
- [Spring Boot Security + JWT Hello World Example](https://www.javainuse.com/spring/boot-jwt)
- [Spring Boot Security + JWT + MySQL Hello World Example](https://www.javainuse.com/spring/boot-jwt-mysql)
- [Spring Boot SSL [https] Example](https://howtodoinjava.com/spring-boot/spring-boot-ssl-https-example/)


## Caching
- [Spring boot caching tutorial with example](https://howtodoinjava.com/spring-boot2/spring-boot-cache-example/)
- [Spring Boot + ehcache 3](https://howtodoinjava.com/spring-boot2/ehcache3-config-example/)
- [A Guide To Caching in Spring](https://www.baeldung.com/spring-cache-tutorial)

## Performance Tuning
- [Comparing Embedded Servlet Containers in Spring Boot](https://www.baeldung.com/spring-boot-servlet-containers)
## Docker - CI/CD
- [Create Docker Images](https://www.tutorialspoint.com/spring_boot/spring_boot_creating_docker_image.htm)
## Maven - Gradle
- [mvn clean install - a short guide to Maven](https://www.marcobehler.com/guides/mvn-clean-install-a-short-guide-to-maven)

## Web Flux
- [Build Reactive REST APIs With Spring WebFlux](https://dzone.com/articles/build-reactive-rest-apis-with-spring-webflux)
- [Getting started with Spring WebFlux](https://jstobigdata.com/spring/getting-started-with-spring-webflux/)
- [Spring WebFlux Tutorial](https://howtodoinjava.com/spring-webflux/spring-webflux-tutorial/)

## MongoDB
- [Spring Boot MongoDB CRUD Example](https://www.devglan.com/spring-boot/spring-boot-mongodb-crud)
- [Spring Data MongoDB Queries](https://www.devglan.com/spring-boot/spring-data-mongodb-queries)